...
def run_delegation_crew(
    ...
    add_event(
        events,
        "executing",
        "Connecting to SQL/CSV MCP server",
        f"Launching {MCP_SERVER_SCRIPT.name} over stdio.",
        "System",
    )

    timeline_placeholder.empty()
    render_event_timeline(events, timeline_placeholder)
    # StdioServerParameters launches mcp_server.py as a subprocess and talks
    # to it over stdin/stdout. MCPServerAdapter is a context manager: tools
    # returned by it are only valid while this "with" block is open, so all
    # agent creation AND crew.kickoff() must happen inside it.
    ...
    with MCPServerAdapter(mcp_server_params) as mcp_tools:
        add_event(
            ...
            "info",
            "MCP tools loaded",
            f"Available tools: {[tool.name for tool in mcp_tools]}",
            "System",
        )
    timeline_placeholder.empty()
    render_event_timeline(events, timeline_placeholder)
    add_event(
            ...
            "Supervisor, Data Scientist, and Data Analyst agents are being initialized with SQL/CSV MCP tools.",
            "System",
        )
    timeline_placeholder.empty()
    render_event_timeline(events, timeline_placeholder)
    ...
    add_event(
        ...
        "CrewAI manager agent can delegate internally to specialist agents, including calls to the SQL/CSV MCP tools.",
        "Supervisor Agent",
    )
    timeline_placeholder.empty()
    render_event_timeline(events, timeline_placeholder)
